...
class StereoSync2(Node):

  def __init__(self):
    super().__init__('stereo_syncronizer2')
    self.sub_info_right = self.create_subscription( CameraInfo, 'carla/ego_vehicle/rgb_right/camera_info', self.listener_right_info_callback, 10)
    self.sub_info_right  # prevent unused variable warning
    self.pub_info_right_ = self.create_publisher( CameraInfo, '/right/camera_info', 10)
    # setup timer
    timer_period = 0.1  # seconds
    self.timer = self.create_timer(timer_period, self.timer_callback)

    from rcl_interfaces.msg import ParameterDescriptor
    baseline_parameter_descriptor = ParameterDescriptor(description='Distance between stereo camera pair')

    # declare baseline parameter
    self.declare_parameter('baseline',
                            0.5,
                            baseline_parameter_descriptor)
